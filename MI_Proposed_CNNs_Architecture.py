...
# DataSet Address
#DIR = 'C:/Users/Snow/Desktop/EEG/transformer/data/10-Subjects-Dataset/10-Subjects-Dataset/R1/'
DIR = '~/Desktop/transformer/transformer/data/R1'
# Model Saver Address
#SAVE = 'C:/Users/Snow/Desktop/EEG/transformer/data/10-Subjects-Dataset/'
SAVE = '~/Desktop/transformer/transformer/data/R1'
# Activate a Session
sess = tfc.InteractiveSession()

# Read Training Data
train_data = pd.read_excel(DIR + 'Training_data.xlsx', header=None)
train_data = np.array(train_data).astype('float32')

# Read Training Labels
train_labels = pd.read_excel(DIR + 'Training_labels.xlsx', header=None)
train_labels = np.array(train_labels)

# Read Testing Data
test_data = pd.read_excel(DIR + 'Test_data.xlsx', header=None)
test_data = np.array(test_data).astype('float32')

# Read Testing Labels
test_labels = pd.read_excel(DIR + 'Test_labels.xlsx', header=None)
...
with tf.name_scope("Input"):
    ...
    with tf.name_scope("Input_Data"):
        x = tfc.placeholder(tf.float32, [None, 640])

    # y is the label related to the data
    with tf.name_scope("Labels"):
        y = tfc.placeholder(tf.float32, [None, 4])
