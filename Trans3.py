...
class channel_attention(nn.Module):
    def __init__(self, sequence_num=40, inter=30):
...
class Trans():
    ...
    def train(self):
        ...
        #test_label = pd.read_excel('./data/10-Subjects-Dataset/10-Subjects-Dataset/L2/Test_labels.xlsx')
        ...
        test_data = torch.as_tensor(np.array(test_data).astype('float'))
        test_label = torch.as_tensor(np.array(test_label).astype('float'))
        test_dataset = torch.utils.data.TensorDataset(test_data, test_label)
        self.test_dataloader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=self.batch_size, shuffle=True)
        test_data = test_data.reshape(-1, 1, 16, 40)
